...
class  adminback
{
    ...
    function delete_catagory($id)
    {
        ...
        if ( mysqli_query($this->connection, $query)) {
            return "{$id} Catagory deleted successfully";
        }
    }
        ...
        if ($img_ext == "jpg" || $img_ext == 'jpeg' || $img_ext == "png") {
            if ($pdt_img_size) {
                ...
                if (mysqli_query($this->connection, $query)) {
                    ...
                    $msg = "Error uploading product: " . mysqli_error($this->connection);
                    return $msg;
                }
            } else {
                $msg = "Image dimensions should be within 271x271 pixels, and file size should not be larger than 2MB";
                return $msg;
            }
        } else {
    ...
    function update_product($data)
    {
        ...
        $pdt_img_tmp = $_FILES['u_pdt_img']['tmp_name'];
            ...
            $queryproduct = "SELECT * FROM `products` WHERE pdt_id =$pdt_id";
            $pdquery =mysqli_query($this->connection, $queryproduct);
            $fetchproduct = mysqli_fetch_assoc($pdquery);
            
            if(isset($fetchproduct['pdt_img']) && !empty($fetchproduct['pdt_img'])){
                $pdt_img_name = $fetchproduct['pdt_img'];
                $img_ext = pathinfo($pdt_img_name, PATHINFO_EXTENSION);
        ...
        if ($img_ext == "jpg" || $img_ext == 'jpeg' || $img_ext == "png") {
            if ($pdt_img_size <= 2e+6) {
                if ($width <= 271 && $height <= 271) {
                    ...
                    $pre_img = $row['pdt_img'];
                    if(isset($row['pdt_img']) && empty($row['pdt_img'])){
                    ...
                    if (mysqli_query($this->connection, $query)) {
                        ...
                        $msg = "Error updating product: " . mysqli_error($this->connection);
                        return $msg;
                    }
                } else {
    ...
    {
        $query = "SELECT * FROM `product_info_ctg` WHERE pdt_id=$cataID ORDER BY pdt_id DESC LIMIT 6";
        if (mysqli_query($this->connection, $query)) {
            $pdt_info = mysqli_query($this->connection, $query);
            return $pdt_info;
        }
    ...
    function productBYcate(){
        $query = "SELECT * FROM `product_info_ctg` WHERE `ctg_id`=2 LIMIT 8";
        if(mysqli_query($this->connection, $query)){
            $result = mysqli_query($this->connection, $query);
            // print_r($result);
            return $result;
